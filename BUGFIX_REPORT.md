...
**7. Code.js - runAllManualMaintenance() に夜間自動リトライトリガー設定を統合**
...
**機能**:
- 「各種設定と書式を再適用」実行時に自動的にトリガーを設定
- 既存トリガーがある場合は再作成しない
- 一度設定すれば毎日午前2時に自動実行され続ける

#### 使用方法

**夜間自動リトライの設定**:
1. カスタムメニュー →「各種設定と書式を再適用」を実行
2. 夜間自動リトライトリガーが自動的に設定されます（毎日午前2時実行）
3. 以降は自動的に毎日午前2時に失敗した同期を再実行します
...
2. removeNightlyRetryTrigger() 関数を選択して実行

#### テスト項目

1. リンク同期を実行 → 一時的なエラーで失敗 → 自動リトライで成功することを確認
2. 失敗した同期を再実行 → 全同期が順番に実行されることを確認
3. 「各種設定と書式を再適用」実行 → トリガーが作成されることを確認（Apps Script → トリガー）
4. トリガー確認 → retryAllFailedSyncs 関数の時間主導型トリガー（毎日午前2時）が表示される
5. 同期失敗履歴を表示 → 失敗した同期が表示されることを確認
6. 複数ユーザーが同時編集 → 30秒のロック待機で処理されることを確認
...
**ファイル**: `Code.js`
**修正日**: 2025-11-05
**修正内容**: `updateInputSheetHeaders()`でヘッダー更新後に列インデックスのキャッシュをクリアする処理を追加
**原因**: ヘッダーを更新しても古い列インデックスがキャッシュされていたため、リンク列が見つからないエラーが発生
**影響**: ヘッダー更新後すぐにリンク同期が正しく動作するようになった

#### 実装詳細

**Code.js - updateInputSheetHeaders()にキャッシュクリア処理を追加 (375-395行目)**
...
**改善点**:
- ヘッダー更新後に該当シートの列インデックスキャッシュを削除
- 次回の`getColumnIndices()`呼び出し時に最新のヘッダーから再計算
- 「リンク列が見つかりません」エラーを解消
